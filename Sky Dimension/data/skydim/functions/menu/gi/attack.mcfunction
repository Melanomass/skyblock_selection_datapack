...
execute if data entity @s attack at @s run particle block minecraft:diamond_block ~ ~0.5 ~ 0.5 0.5 0.5 1 10 normal
# Score Change
execute run scoreboard players add ps gg 1
execute run scoreboard players set ps ng 1
execute run scoreboard players set ps eg 1
execute if score ps gg > y_gg gg run scoreboard players set ps gg 1

execute if score ps gg = y_gg gg at @s run data modify entity @e[limit=1,sort=nearest,distance=..10,tag=skydimension.gigantic-option,type=minecraft:text_display] text set value '{"text":"Yes","color":"green","bold":true}'
